...
namespace CoreCourse.StateMgmt.Web.Services
{
    public class PICalculator
    {
        public async Task<int> GetNineDigits(int start)
        {
            return await Task.Factory.StartNew(() => { return startingAt(start + 1 ); });
        }

        private int mul_mod(long a, long b, int m)
        {
            return (int)((a * b) % m);
        }

        // return the inverse of x mod y
        private int inv_mod(int x, int y)
        {
            int q = 0;
            int u = x;
            int v = y;
        ...
        // return (a^b) mod m
        private int pow_mod(int a, int b, int m)
        {
            int r = 1;
            int aa = a;
